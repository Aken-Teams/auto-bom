...
> 之后步骤 ④⑤⑥ 产出 C-CMAX 导入清单与 3 份 ERP 上传档（pj_bom_loader / routings / sequences）。
...
### 5️⃣ pj_bom_loader 的「ERP 数量」栏

焊接 / 成型 / 包装罐的数量都是 1（已正确）；只有**切割（WAF 晶片）那一行**是「单位耗用量」，会是 0 或 1 不等。料号清单里已有「单位用量」栏。
